+++
date = '2026-05-20T18:35:00+02:00'
draft = false
title = 'Émi Lefèvre — CV'
+++

**Gamedev généraliste** basée à **Montpellier**, avec une expérience en direction créative, game design, production, prototypage, outils, art 3D et gestion de studio.

## Expérience

### [Caravan SandWitch](https://caravansandwitch.com/) - cofondatrice, direction créative, gamedev généraliste

Projet lancé en 2021, sorti en septembre 2024 sur **Steam, Nintendo Switch et PlayStation 5**. Gestion d'une équipe de **15 personnes**.

- **Direction créative** et prototypage initial.
- **Game design**, level design, narrative design, quest design et world design.
- **Écriture** de dialogues et travail sur les cinématiques.
- **Modèles 3D** de personnages, props et environment art.
- **Tooling**, prototypes de gameplay, **DevOps**, pipelines de build, déploiement et localisation.
- **Marketing**, community management et communication de fin de production.
- **Gestion de studio et de projet** : recrutement, planning agile, Scrum, relations éditeur, partenaires, financements.
- Crédit complet de l'équipe disponible sur [MobyGames](https://www.mobygames.com/game/230447/caravan-sandwitch/credits/windows/).

## Outils et technologies

- **Unreal Engine 5**, **C++** et **Blueprint** pour du prototypage/tooling.
- **Unity** et **C#**.
- **Blender**, modélisation, rigging.
- **Figma**, **Notion**.
- **Python**, CI/CD **GitLab**.
- **Proxmox** et **NixOS** pour des projets personnels d'infrastructure.

## Formation

- **Prépa scientifique**, 2017-2019.
- **École d'ingénieur** ENSGSI, 2019-2021.
- **Stage** à [Sorbonne Université sur le projet Ikigai](https://www.sorbonne-universite.fr/actualites/ikigai-la-plateforme-de-jeux-video-qui-transforme-lapprentissage), comme **développeuse gameplay Unity/C#** sur le jeu éducatif [Stranger Fields](https://ikigai.games/games/gameDetails/strangerfields).

## Conférences et interviews

- Intervention sur le [level design et world design à Toulouse GameDev](https://www.youtube.com/watch?v=vsKJmJjsRk0).
- Interview chez [Origami](https://www.youtube.com/watch?v=386XlRXUTUU) autour du parcours, de Caravan SandWitch et des [Pégases](https://www.bfmtv.com/tech/gaming/pegases-2025-du-jeu-video-prince-of-persia-et-caravan-sandwitch-en-tete-des-nominations_AN-202502030448.html).
- Autres conférences et interview à Spawn, Gameconf, Lyon game dev, ICCARE, radio Clapas.

## Activités actuelles

Projets personnels, jeux plus petits, photo, conférences, auto-hébergement, et engagements associatifs autour du **soutien communautaire trans** à Montpellier.

## Contact

{{< mail >}} · [LinkedIn](https://www.linkedin.com/in/emi-le/) · [Bluesky](https://bsky.app/profile/laitue.bsky.social)
